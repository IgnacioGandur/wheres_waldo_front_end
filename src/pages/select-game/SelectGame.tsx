...
import SelectGameBackground from "./SelectGameBackground";
...
const SelectGame = () => {
    ...
    return (
        <>
            ...
            <main className={styles["select-game"]}>
                ...
                <div className={styles["content"]}>
                    <header className={styles["header"]}>
                        {loaderData.fail && (
                            <p className={styles.error}>{loaderData.error}</p>
                        )}
                        ...
                        {loaderData.games.map((game: any, index: number) => {
                            return (
                                <NavLink
                                    ...
                                    to={`/games/${game.slug}`}
                                    style={{
                                        animationDelay: `${(index + 1) * 3}00ms`,
                                    }}
                                >
                                    <img
                                        className={styles["background-image"]}
                                        src={
                                            `/images/previews/${game.filename}`.split(
                                                ".",
                                            )[0] + ".jpg"
                                        }
                                        alt={game.name}
