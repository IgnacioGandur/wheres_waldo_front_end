import styles from "./About.module.css";
import Background from "./Background";


type SectionType = {
    title: string;
    details: string;
    icon: TrustedHTML;
};

const sections: SectionType[] = [
    {
        title: "What is this project?",
        details: `The purpose of this "Where's Waldo" style project was to consolidate my learning and understanding of how front-end applications interact with a back-end that receives requests, validates them, process them and return a response accordingly.`,
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><path fill="currentColor" d="M39.195 37.311c-4.338-2.949-9.924-3.91-15.049-2.584c-1.35.365-.359 3.523.904 3.201c4.719-1.201 9.277.141 12.143 2.084c1.065.75 3.073-1.955 2.002-2.701m10.49-20.273l-.139-.207l-.235-.083c-3.83-1.356-8.632-1.501-12.7-.363l-.244.068l-.152.202a8.2 8.2 0 0 0-1.675 4.974c0 4.559 3.709 8.268 8.268 8.268s8.27-3.709 8.27-8.268a8.25 8.25 0 0 0-1.393-4.591m-6.878 11.358a6.776 6.776 0 0 1-6.768-6.768c0-1.25.358-2.444 1.01-3.506c-.018.144-.051.281-.051.43a3.76 3.76 0 0 0 7.52 0c0-.477-.1-.927-.263-1.346c1.507.133 2.975.422 4.315.872a6.73 6.73 0 0 1 1.006 3.55a6.776 6.776 0 0 1-6.769 6.768m-13.346-6.767a8.23 8.23 0 0 0-1.392-4.591l-.139-.207l-.235-.083c-3.83-1.356-8.632-1.501-12.7-.363l-.244.068l-.153.202a8.2 8.2 0 0 0-1.675 4.974c0 4.559 3.709 8.268 8.268 8.268c4.56-.001 8.27-3.709 8.27-8.268m-8.27 6.767a6.776 6.776 0 0 1-6.768-6.768c0-1.25.358-2.444 1.01-3.506c-.018.144-.051.281-.051.43a3.76 3.76 0 0 0 7.52 0c0-.477-.1-.927-.263-1.346c1.507.133 2.975.422 4.315.872a6.73 6.73 0 0 1 1.006 3.55c.001 3.732-3.035 6.768-6.769 6.768"></path><path fill="currentColor" d="M32 2C16.531 2 4 14.533 4 30a27.83 27.83 0 0 0 3.895 14.209c-.392.896-.744 1.848-.791 3.047q-.016.305-.037.654c-.135 2.057-.334 5.166 2.416 9.063c1.775 2.512 4.531 4.162 7.969 4.766c1.219.213 2.467.295 3.822.25c2.217-.076 3.773-.156 4.625-1.703c.722-1.309-.109-2.135.507-2.856A28 28 0 0 0 32 58c15.469 0 28-12.533 28-28S47.469 2 32 2m-6.989 55.969c.277 1.699-.689 3.305-7.216 2.127c-10.741-1.938-10.012-12.1-8.277-15.761c1.274-2.689 1.08-3.833.344-7.171c-.669-3.029 5.053-1.658 5.217 1.974c.121 2.694-.916 4.068.375 5.253c2.886 2.65 11.213 1.198 16.842.004c2.408-.511 3.646 1.931 1.861 3.175c-1.908 1.333-4.139 1.874-7.439 2.116c-1.723.125.221.68-.049 2.316c-.147.898-1.088.867-1.052 1.801c.019.497 1.044 1.02.347 2.27c-.207.373-1.133.801-.953 1.896M32 55.5c-1.536 0-3.037-.147-4.499-.41a2.7 2.7 0 0 0-.278-.83c-.377-.738 1.498-1.5.736-3.482c2.857-.469 4.982-1.02 6.951-2.139c2.117-1.203 2.188-2.514 2.043-3.188c-.246-1.123-1.404-2.049-2.887-2.309a4.2 4.2 0 0 0-1.686.037c-3.125.758-10.604 1.779-14.387 1.115c-1.063-.188-1.41-.441-1.508-.541c-.549-.551-.445-1.605-.176-3.785c.33-2.68-2.078-5.537-4.213-5.912l-.137-.023c-1.178-.148-2.229.191-2.883.934c-.732.826-.896 2.061-.453 3.383c.349 1.039.428 1.873.36 2.615A25.4 25.4 0 0 1 6.5 30c0-7.028 2.858-13.401 7.473-18.018c.079.022.152.022.213-.015a13.28 13.28 0 0 1 11.256-.983c.512.198 1.328-1.683.703-1.92a15.3 15.3 0 0 0-7.538-.752A25.34 25.34 0 0 1 32 4.5c7.063 0 13.464 2.889 18.087 7.544a15.33 15.33 0 0 0-12.929 1.157c-.572.342.557 2.049 1.029 1.768a13.28 13.28 0 0 1 11.256-.986c.429.167 1.06-1.114.89-1.68C54.765 16.893 57.5 23.131 57.5 30c0 14.061-11.439 25.5-25.5 25.5"></path></svg>',
    },
    {
        title: "Project goal",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path fill="currentColor" d="M8 14A6 6 0 1 0 8 2a6 6 0 0 0 0 12m0 1A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0-3a4 4 0 1 0 0-8a4 4 0 0 0 0 8m0-1a3 3 0 1 1 0-6a3 3 0 0 1 0 6m0-2a1 1 0 1 0 0-2a1 1 0 0 0 0 2"></path></svg>',
        details: "The main goal of this project was to learn about testing in the backend with testing tools like Vitest, Supertest and Jest matchers.",
    },
    {
        title: "Notes",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="currentColor" d="M112 152h288v32H112zm0 88h288v32H112zm0 88h152v32H112z"></path><path fill="currentColor" d="M480 48H32v416h448Zm-32 384H64V80h384Z"></path></svg>',
        details: "1. The front-end, back-end and database for this project are hosted in free-tier services and that can impact the loading times. 2. This is my first time using Typescript in a project. 3. Almost all the animations in this project were made by my with vanilla CSS, I used React motion to animate the unmounting of components."
    },
    {
        title: "Author",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="currentColor" d="M94.055 21.9L18.998 96.96l42.727 23.6l-26.98 26.952L142.35 212.39c-40.443 70.148-30.72 161.07 29.2 220.958c71.605 71.606 187.737 71.587 259.356 0c71.62-71.587 71.642-187.654.037-259.22c-59.915-59.878-150.896-69.57-221.084-29.177L144.95 37.415l-8.44 8.432l-18.588 18.57zm47.224 45.598l62.337 103.275l8.098-5.248c44.21-28.663 99.014-34.044 147.166-16.078q-1.74-.04-3.503-.04c-38.988 0-70.594 14.807-70.594 33.073c0 18.27 31.606 33.075 70.594 33.075c31.53 0 58.225-9.684 67.287-23.05c15.942 17.34 27.492 37.224 34.65 58.253c-7.76-3.387-18.28-6.706-30.902-9.563c-31.383-7.1-75.547-11.615-124.305-11.615c-48.757 0-92.92 4.514-124.304 11.615c-13.71 3.102-24.997 6.75-32.893 10.438a164 164 0 0 1 18.018-37.383l5.263-8.104l-103.33-62.3l13.894-13.88l46.937 25.923l27.914-27.915l-26.18-46.635l13.855-13.842zm-1.087 201.287q.722.42 1.506.84c7.89 4.22 20.41 8.487 36.103 12.037c31.383 7.1 75.547 11.615 124.304 11.615s92.922-4.514 124.305-11.615c15.687-3.55 28.203-7.813 36.094-12.033a164 164 0 0 1 2.746 17.643c-9.432 4.277-21.204 7.893-35.074 11.032c-33.205 7.513-78.27 12.037-128.07 12.037c-49.802 0-94.866-4.524-128.07-12.037c-14.67-3.32-27-7.17-36.69-11.776a165 165 0 0 1 2.845-17.745z"></path></svg>',
        details: "Ignacio Gandur"
    }
];

type Tool = {
    name: string;
    icon: String;
    description: string;
    link: string;
    end: string;
};


const tools: Tool[] = [
    {
        name: "Motion",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="#f6eb2a" d="M8 17q-.425 0-.712-.288T7 16t.288-.712T8 15h5q-.35-.425-.562-.925T12.1 13H10q-.425 0-.712-.288T9 12t.288-.712T10 11h2.1q.125-.575.338-1.075T13 9H4q-.425 0-.712-.288T3 8t.288-.712T4 7h13q2.075 0 3.538 1.463T22 12t-1.463 3.538T17 17zm9-2q1.25 0 2.125-.875T20 12t-.875-2.125T17 9t-2.125.875T14 12t.875 2.125T17 15M3 13q-.425 0-.712-.288T2 12t.288-.712T3 11h4q.425 0 .713.288T8 12t-.288.713T7 13zm1 4q-.425 0-.712-.288T3 16t.288-.712T4 15h1q.425 0 .713.288T6 16t-.288.713T5 17z"></path></svg>',
        description: "An animation library to make animations for React components (I used it mainly to animate unmounting components).",
        link: "https://motion.dev/",
        end: "front-end"
    },
    {
        name: "Vite",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><g fill="none"><path fill="url(#vscodeIconsFileTypeVite0)" d="m29.884 6.146l-13.142 23.5a.714.714 0 0 1-1.244.005L2.096 6.148a.714.714 0 0 1 .746-1.057l13.156 2.352a.7.7 0 0 0 .253 0l12.881-2.348a.714.714 0 0 1 .752 1.05z"/><path fill="url(#vscodeIconsFileTypeVite1)" d="M22.264 2.007L12.54 3.912a.36.36 0 0 0-.288.33l-.598 10.104a.357.357 0 0 0 .437.369l2.707-.625a.357.357 0 0 1 .43.42l-.804 3.939a.357.357 0 0 0 .454.413l1.672-.508a.357.357 0 0 1 .454.414l-1.279 6.187c-.08.387.435.598.65.267l.143-.222l7.925-15.815a.357.357 0 0 0-.387-.51l-2.787.537a.357.357 0 0 1-.41-.45l1.818-6.306a.357.357 0 0 0-.412-.45"/><defs><linearGradient id="vscodeIconsFileTypeVite0" x1="6" x2="235" y1="33" y2="344" gradientTransform="translate(1.34 1.894)scale(.07142)" gradientUnits="userSpaceOnUse"><stop stop-color="#41d1ff"/><stop offset="1" stop-color="#bd34fe"/></linearGradient><linearGradient id="vscodeIconsFileTypeVite1" x1="194.651" x2="236.076" y1="8.818" y2="292.989" gradientTransform="translate(1.34 1.894)scale(.07142)" gradientUnits="userSpaceOnUse"><stop stop-color="#ffea83"/><stop offset=".083" stop-color="#ffdd35"/><stop offset="1" stop-color="#ffa800"/></linearGradient></defs></g></svg>',
        description: "Front-end development tool that powered this project's development",
        link: "https://vite.dev/",
        end: "front-end"
    },
    {
        name: "React",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128"><g fill="#61dafb"><circle cx="64" cy="64" r="11.4"/><path d="M107.3 45.2c-2.2-.8-4.5-1.6-6.9-2.3c.6-2.4 1.1-4.8 1.5-7.1c2.1-13.2-.2-22.5-6.6-26.1c-1.9-1.1-4-1.6-6.4-1.6c-7 0-15.9 5.2-24.9 13.9c-9-8.7-17.9-13.9-24.9-13.9c-2.4 0-4.5.5-6.4 1.6c-6.4 3.7-8.7 13-6.6 26.1c.4 2.3.9 4.7 1.5 7.1c-2.4.7-4.7 1.4-6.9 2.3C8.2 50 1.4 56.6 1.4 64s6.9 14 19.3 18.8c2.2.8 4.5 1.6 6.9 2.3c-.6 2.4-1.1 4.8-1.5 7.1c-2.1 13.2.2 22.5 6.6 26.1c1.9 1.1 4 1.6 6.4 1.6c7.1 0 16-5.2 24.9-13.9c9 8.7 17.9 13.9 24.9 13.9c2.4 0 4.5-.5 6.4-1.6c6.4-3.7 8.7-13 6.6-26.1c-.4-2.3-.9-4.7-1.5-7.1c2.4-.7 4.7-1.4 6.9-2.3c12.5-4.8 19.3-11.4 19.3-18.8s-6.8-14-19.3-18.8M92.5 14.7c4.1 2.4 5.5 9.8 3.8 20.3c-.3 2.1-.8 4.3-1.4 6.6c-5.2-1.2-10.7-2-16.5-2.5c-3.4-4.8-6.9-9.1-10.4-13c7.4-7.3 14.9-12.3 21-12.3c1.3 0 2.5.3 3.5.9M81.3 74c-1.8 3.2-3.9 6.4-6.1 9.6c-3.7.3-7.4.4-11.2.4c-3.9 0-7.6-.1-11.2-.4q-3.3-4.8-6-9.6c-1.9-3.3-3.7-6.7-5.3-10c1.6-3.3 3.4-6.7 5.3-10c1.8-3.2 3.9-6.4 6.1-9.6c3.7-.3 7.4-.4 11.2-.4c3.9 0 7.6.1 11.2.4q3.3 4.8 6 9.6c1.9 3.3 3.7 6.7 5.3 10c-1.7 3.3-3.4 6.6-5.3 10m8.3-3.3c1.5 3.5 2.7 6.9 3.8 10.3c-3.4.8-7 1.4-10.8 1.9c1.2-1.9 2.5-3.9 3.6-6c1.2-2.1 2.3-4.2 3.4-6.2M64 97.8c-2.4-2.6-4.7-5.4-6.9-8.3c2.3.1 4.6.2 6.9.2s4.6-.1 6.9-.2c-2.2 2.9-4.5 5.7-6.9 8.3m-18.6-15c-3.8-.5-7.4-1.1-10.8-1.9c1.1-3.3 2.3-6.8 3.8-10.3c1.1 2 2.2 4.1 3.4 6.1c1.2 2.2 2.4 4.1 3.6 6.1m-7-25.5c-1.5-3.5-2.7-6.9-3.8-10.3c3.4-.8 7-1.4 10.8-1.9c-1.2 1.9-2.5 3.9-3.6 6c-1.2 2.1-2.3 4.2-3.4 6.2M64 30.2c2.4 2.6 4.7 5.4 6.9 8.3c-2.3-.1-4.6-.2-6.9-.2s-4.6.1-6.9.2c2.2-2.9 4.5-5.7 6.9-8.3m22.2 21l-3.6-6c3.8.5 7.4 1.1 10.8 1.9c-1.1 3.3-2.3 6.8-3.8 10.3c-1.1-2.1-2.2-4.2-3.4-6.2M31.7 35c-1.7-10.5-.3-17.9 3.8-20.3c1-.6 2.2-.9 3.5-.9c6 0 13.5 4.9 21 12.3c-3.5 3.8-7 8.2-10.4 13c-5.8.5-11.3 1.4-16.5 2.5c-.6-2.3-1-4.5-1.4-6.6M7 64c0-4.7 5.7-9.7 15.7-13.4c2-.8 4.2-1.5 6.4-2.1c1.6 5 3.6 10.3 6 15.6c-2.4 5.3-4.5 10.5-6 15.5C15.3 75.6 7 69.6 7 64m28.5 49.3c-4.1-2.4-5.5-9.8-3.8-20.3c.3-2.1.8-4.3 1.4-6.6c5.2 1.2 10.7 2 16.5 2.5c3.4 4.8 6.9 9.1 10.4 13c-7.4 7.3-14.9 12.3-21 12.3c-1.3 0-2.5-.3-3.5-.9M96.3 93c1.7 10.5.3 17.9-3.8 20.3c-1 .6-2.2.9-3.5.9c-6 0-13.5-4.9-21-12.3c3.5-3.8 7-8.2 10.4-13c5.8-.5 11.3-1.4 16.5-2.5c.6 2.3 1 4.5 1.4 6.6m9-15.6c-2 .8-4.2 1.5-6.4 2.1c-1.6-5-3.6-10.3-6-15.6c2.4-5.3 4.5-10.5 6-15.5c13.8 4 22.1 10 22.1 15.6c0 4.7-5.8 9.7-15.7 13.4"/></g></svg>',
        description: "A front-end Javascript library to build user interfaces",
        link: "https://react.dev/",
        end: "front-end"
    },
    {
        name: "React Router",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128"><path fill="#f44250" d="M99.04 50.315c-3.646-1.148-5.528-.761-9.351-1.18c-5.91-.645-8.265-2.959-9.264-8.913c-.559-3.324.101-8.191-1.326-11.204c-2.729-5.746-9.115-8.323-15.53-6.852c-5.417 1.244-9.924 6.666-10.174 12.232c-.285 6.355 3.33 11.778 9.537 13.65c2.95.89 6.083 1.338 9.163 1.565c5.649.413 6.083 3.558 7.61 6.147c.964 1.632 1.897 3.24 1.897 8.107s-.939 6.474-1.896 8.106c-1.528 2.583-3.413 4.275-9.062 4.693c-3.08.228-6.22.676-9.162 1.567c-6.208 1.877-9.823 7.293-9.538 13.649c.25 5.566 4.757 10.988 10.174 12.232c6.415 1.476 12.801-1.106 15.53-6.852c1.433-3.013 2.218-6.427 2.777-9.75c1.005-5.955 3.36-8.269 9.264-8.915c3.823-.418 7.78 0 11.363-2.038c3.785-2.77 7.168-6.93 7.168-12.692c0-5.764-3.645-11.806-9.18-13.552"/><path fill="#121212" d="M38.4 77.373c-7.45 0-13.528-6.098-13.528-13.575s6.077-13.574 13.527-13.574s13.528 6.098 13.528 13.574c0 7.47-6.083 13.575-13.528 13.575zM13.48 104.96C6.04 104.942-.018 98.82 0 91.337c.018-7.47 6.119-13.55 13.575-13.526c7.444.018 13.503 6.14 13.48 13.622c-.018 7.464-6.12 13.545-13.575 13.527m100.512 0c-7.462.041-13.586-6.015-13.629-13.473c-.041-7.488 5.994-13.634 13.426-13.676c7.462-.042 13.587 6.014 13.629 13.472c.041 7.482-5.994 13.635-13.426 13.677"/></svg>',
        description: "A routing tool used alongside React, to provide a fluid user interaction through the app's pages",
        link: "https://reactrouter.com/",
        end: "front-end"
    },
    {
        name: "React Spinners",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 3v3m6.366-.366l-2.12 2.12M21 12h-3m.366 6.366l-2.12-2.12M12 21v-3m-6.366.366l2.12-2.12M3 12h3m-.366-6.366l2.12 2.12"/></svg>',
        description: "A library that provides good looking loader components",
        link: "https://www.davidhu.io/react-spinners/",
        end: "front-end"
    },
    {
        name: "CSS",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128"><path fill="#1572b6" d="M18.814 114.123L8.76 1.352h110.48l-10.064 112.754l-45.243 12.543z"/><path fill="#33a9dc" d="m64.001 117.062l36.559-10.136l8.601-96.354h-45.16z"/><path fill="#fff" d="M64.001 51.429h18.302l1.264-14.163H64.001V23.435h34.682l-.332 3.711l-3.4 38.114h-30.95z"/><path fill="#ebebeb" d="m64.083 87.349l-.061.018l-15.403-4.159l-.985-11.031H33.752l1.937 21.717l28.331 7.863l.063-.018z"/><path fill="#fff" d="m81.127 64.675l-1.666 18.522l-15.426 4.164v14.39l28.354-7.858l.208-2.337l2.406-26.881z"/><path fill="#ebebeb" d="M64.048 23.435v13.831H30.64l-.277-3.108l-.63-7.012l-.331-3.711zm-.047 27.996v13.831H48.792l-.277-3.108l-.631-7.012l-.33-3.711z"/></svg>',
        description: "The language used to style the entirety of this page and most of its animations (no framework)",
        link: "https://developer.mozilla.org/en-US/docs/Web/CSS",
        end: "front-end"
    },
    {
        name: "Date-fns",
        icon: `<svg viewBox="0 0 163 136" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:sketch="http://www.bohemiancoding.com/sketch/ns">
    <!-- Generator: Sketch 3.3.3 (12072) - http://www.bohemiancoding.com/sketch -->
    <title>date-fns-transparent copy 2</title>
    <desc>Created with Sketch.</desc>
    <defs></defs>
    <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd" sketch:type="MSPage">
        <g id="Solid-logo" sketch:type="MSLayerGroup" transform="translate(-19.000000, -32.000000)" fill="#FFFFFF">
            <g id="White-logo" transform="translate(19.000000, 32.000000)" sketch:type="MSShapeGroup">
                <path d="M0.4,67.8 C0.4,54.7439347 2.799976,42.4320578 7.6,30.864 C12.400024,19.2959422 19.3119549,9.04804464 28.336,0.12 L46.48,0.12 C27.6639059,18.7440931 18.256,41.3038675 18.256,67.8 C18.256,94.2961325 27.6639059,116.855907 46.48,135.48 L28.336,135.48 C19.3119549,126.551955 12.400024,116.304058 7.6,104.736 C2.799976,93.1679422 0.4,80.8560653 0.4,67.8 L0.4,67.8 Z M162.6,67.8 C162.6,80.8560653 160.200024,93.1679422 155.4,104.736 C150.599976,116.304058 143.688045,126.551955 134.664,135.48 L116.52,135.48 C135.336094,116.855907 144.744,94.2961325 144.744,67.8 C144.744,41.3038675 135.336094,18.7440931 116.52,0.12 L134.664,0.12 C143.688045,9.04804464 150.599976,19.2959422 155.4,30.864 C160.200024,42.4320578 162.6,54.7439347 162.6,67.8 L162.6,67.8 Z" id="Parans"></path>
                <g id="Hands" transform="translate(67.000000, 9.000000)">
                    <rect id="Hand" x="0" y="0" width="12" height="69"></rect>
                    <rect id="Hand" transform="translate(32.500000, 40.500000) scale(-1, 1) rotate(-45.000000) translate(-32.500000, -40.500000) " x="27" y="6" width="11" height="69"></rect>
                </g>
            </g>
        </g>
    </g>
</svg>`,
        description: "My favorite Javascript library to format dates.",
        link: "https://date-fns.org/",
        end: "front-end"
    },
    {
        name: "Node",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128"><path fill="url(#deviconNodejs0)" d="M66.958.825a6.07 6.07 0 0 0-6.035 0L11.103 29.76c-1.895 1.072-2.96 3.095-2.96 5.24v57.988c0 2.143 1.183 4.167 2.958 5.24l49.82 28.934a6.07 6.07 0 0 0 6.036 0l49.82-28.935c1.894-1.072 2.958-3.096 2.958-5.24V35c0-2.144-1.183-4.167-2.958-5.24z"/><path fill="url(#deviconNodejs1)" d="M116.897 29.76L66.841.825A8 8 0 0 0 65.302.23L9.21 96.798a6.3 6.3 0 0 0 1.657 1.43l50.057 28.934c1.42.833 3.076 1.072 4.615.595l52.66-96.925a3.7 3.7 0 0 0-1.302-1.072"/><path fill="url(#deviconNodejs2)" d="M116.898 98.225c1.42-.833 2.485-2.262 2.958-3.81L65.066.108c-1.42-.238-2.959-.119-4.26.715L11.104 29.639l53.606 98.355c.71-.12 1.54-.358 2.25-.715z"/><defs><linearGradient id="deviconNodejs0" x1="34.513" x2="27.157" y1="15.535" y2="30.448" gradientTransform="translate(-129.242 -73.715)scale(6.18523)" gradientUnits="userSpaceOnUse"><stop stop-color="#3f873f"/><stop offset=".33" stop-color="#3f8b3d"/><stop offset=".637" stop-color="#3e9638"/><stop offset=".934" stop-color="#3da92e"/><stop offset="1" stop-color="#3dae2b"/></linearGradient><linearGradient id="deviconNodejs1" x1="30.009" x2="50.533" y1="23.359" y2="8.288" gradientTransform="translate(-129.242 -73.715)scale(6.18523)" gradientUnits="userSpaceOnUse"><stop offset=".138" stop-color="#3f873f"/><stop offset=".402" stop-color="#52a044"/><stop offset=".713" stop-color="#64b749"/><stop offset=".908" stop-color="#6abf4b"/></linearGradient><linearGradient id="deviconNodejs2" x1="21.917" x2="40.555" y1="22.261" y2="22.261" gradientTransform="translate(-129.242 -73.715)scale(6.18523)" gradientUnits="userSpaceOnUse"><stop offset=".092" stop-color="#6abf4b"/><stop offset=".287" stop-color="#64b749"/><stop offset=".598" stop-color="#52a044"/><stop offset=".862" stop-color="#3f873f"/></linearGradient></defs></svg>',
        description: "The runtime environment that powers the API",
        link: "https://nodejs.org/es",
        end: "back-end"
    },
    {
        name: "Npm",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128"><path fill="#cb3837" d="M0 7.062C0 3.225 3.225 0 7.062 0h113.88c3.838 0 7.063 3.225 7.063 7.062v113.88c0 3.838-3.225 7.063-7.063 7.063H7.062c-3.837 0-7.062-3.225-7.062-7.063zm23.69 97.518h40.395l.05-58.532h19.494l-.05 58.581h19.543l.05-78.075l-78.075-.1l-.1 78.126z"/><path fill="#fff" d="M25.105 65.52V26.512H40.96c8.72 0 26.274.034 39.008.075l23.153.075v77.866H83.645v-58.54H64.057v58.54H25.105z"/></svg>',
        description: "The package manager to install the needed packages",
        link: "https://www.npmjs.com/",
        end: "back-end"
    },
    {
        name: "Express",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" d="M24 18.588a1.53 1.53 0 0 1-1.895-.72l-3.45-4.771l-.5-.667l-4.003 5.444a1.466 1.466 0 0 1-1.802.708l5.158-6.92l-4.798-6.251a1.595 1.595 0 0 1 1.9.666l3.576 4.83l3.596-4.81a1.435 1.435 0 0 1 1.788-.668L21.708 7.9l-2.522 3.283a.666.666 0 0 0 0 .994l4.804 6.412zM.002 11.576l.42-2.075c1.154-4.103 5.858-5.81 9.094-3.27c1.895 1.489 2.368 3.597 2.275 5.973H1.116C.943 16.447 4.005 19.009 7.92 17.7a4.08 4.08 0 0 0 2.582-2.876c.207-.666.548-.78 1.174-.588a5.42 5.42 0 0 1-2.589 3.957a6.27 6.27 0 0 1-7.306-.933a6.58 6.58 0 0 1-1.64-3.858c0-.235-.08-.455-.134-.666A88 88 0 0 1 0 11.577zm1.127-.286h9.654c-.06-3.076-2.001-5.258-4.59-5.278c-2.882-.04-4.944 2.094-5.071 5.264z"></path></svg>',
        description: "The web framework in which the API is built",
        link: "https://www.npmjs.com/package/express",
        end: "back-end"
    },
    {
        name: "Prisma ORM",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" d="M4.003 15.728a1 1 0 0 1-.008-1.054l7.479-12.199a.996.996 0 0 1 1.753.104l6.832 14.82a.996.996 0 0 1-.618 1.37l-10.628 3.19a.996.996 0 0 1-1.127-.42zm8.334-9.686L9.707 19.14a.373.373 0 0 0 .472.431l7.343-2.192a.373.373 0 0 0 .236-.506L13.046 5.968a.373.373 0 0 0-.71.074"/></svg>',
        description: "An Object Relational Mapper tool that creates an API for your project's relational database",
        link: "https://www.prisma.io/",
        end: "back-end"
    },
    {
        name: "Nodemon",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><path fill="#8bc34a" d="m27.354 8.517l-10.708-6.34A1.27 1.27 0 0 0 15.999 2a1.27 1.27 0 0 0-.647.178L4.647 8.516A1.33 1.33 0 0 0 4 9.664v12.678a1.33 1.33 0 0 0 .647 1.147l10.705 6.333a1.26 1.26 0 0 0 1.293 0l10.708-6.333A1.33 1.33 0 0 0 28 22.341V9.664a1.33 1.33 0 0 0-.646-1.147M24 23.307a.5.5 0 0 1-.658.474l-5-1.667A.5.5 0 0 1 18 21.64V15.6a.5.5 0 0 0-.223-.415l-1.5-1a.5.5 0 0 0-.554 0l-1.5 1A.5.5 0 0 0 14 15.6v6.039a.5.5 0 0 1-.342.474l-5 1.667A.5.5 0 0 1 8 23.306V12.31a.5.5 0 0 1 .276-.447l1.942-.971A2 2 0 0 1 10 10V6l3.333 3.333l2.443-1.221a.5.5 0 0 1 .448 0l2.443 1.221L22 6v4a2 2 0 0 1-.218.89l1.942.972a.5.5 0 0 1 .276.447Z"/></svg>',
        description: "A Node development tool",
        link: "https://www.npmjs.com/package/nodemon",
        end: "back-end"
    },
    {
        name: "CORS",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48"><g fill="none" stroke="currentColor" stroke-linejoin="round" stroke-width="4"><path d="M37 44a4 4 0 1 0 0-8a4 4 0 0 0 0 8ZM11 12a4 4 0 1 0 0-8a4 4 0 0 0 0 8Zm0 32a4 4 0 1 0 0-8a4 4 0 0 0 0 8Z"/><path stroke-linecap="round" d="M11 12v24m13-26h9a4 4 0 0 1 4 4v22"/><path stroke-linecap="round" d="m30 16l-6-6l6-6"/></g></svg>',
        description: "Middleware library to enable CORS protocol in the API",
        link: "https://www.npmjs.com/package/cors",
        end: "back-end"
    },
    {
        name: "Dotenv",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><g fill-rule="evenodd"><path d="M3.167 3.167h25.667v25.667H3.167z"/><path fill="#ecd53f" fill-rule="nonzero" d="M30 2v28H2V2zM14.757 20.539H9.98v6.44h4.898v-1.085h-3.597V24.14h3.232v-1.085H11.28v-1.428h3.475v-1.09zm2.503 0h-1.264v6.44h1.207v-4.2l2.597 4.2h1.305v-6.44h-1.21v4.3zm5.97 0h-1.41l2.303 6.44h1.388l2.306-6.44h-1.38l-1.577 4.766l-1.63-4.767zM8.3 24.96H6.34v1.96H8.3z"/></g></svg>',
        description: "A library to use environment variables in the API",
        link: "https://www.npmjs.com/package/dotenv",
        end: "back-end"
    },
    {
        name: "Express Validator",
        icon: `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 375 375" version="1.1">
    <defs>
        <clipPath id="clip1">
            <path d="M 18.75 18.75 L 356.25 18.75 L 356.25 356.25 L 18.75 356.25 Z M 18.75 18.75 " />
        </clipPath>
    </defs>
    <g id="surface1">
        <g clip-path="url(#clip1)" clip-rule="nonzero">
            <path style="stroke:none;fill-rule:nonzero;fill:rgb(154, 0, 255);fill-opacity:1;" d="M 356.25 187.5 C 356.25 198.582031 355.167969 209.554688 353.007812 220.421875 C 350.847656 231.289062 347.644531 241.839844 343.40625 252.078125 C 339.164062 262.316406 333.964844 272.039062 327.808594 281.253906 C 321.65625 290.464844 314.660156 298.988281 306.824219 306.824219 C 298.988281 314.660156 290.464844 321.65625 281.253906 327.808594 C 272.039062 333.964844 262.316406 339.164062 252.078125 343.40625 C 241.839844 347.644531 231.289062 350.847656 220.421875 353.007812 C 209.554688 355.167969 198.582031 356.25 187.5 356.25 C 176.417969 356.25 165.445312 355.167969 154.578125 353.007812 C 143.710938 350.847656 133.160156 347.644531 122.921875 343.40625 C 112.683594 339.164062 102.960938 333.964844 93.746094 327.808594 C 84.535156 321.65625 76.011719 314.660156 68.175781 306.824219 C 60.339844 298.988281 53.34375 290.464844 47.191406 281.253906 C 41.035156 272.039062 35.835938 262.316406 31.59375 252.078125 C 27.355469 241.839844 24.152344 231.289062 21.992188 220.421875 C 19.832031 209.554688 18.75 198.582031 18.75 187.5 C 18.75 176.417969 19.832031 165.445312 21.992188 154.578125 C 24.152344 143.710938 27.355469 133.160156 31.59375 122.921875 C 35.835938 112.683594 41.035156 102.960938 47.191406 93.746094 C 53.34375 84.535156 60.339844 76.011719 68.175781 68.175781 C 76.011719 60.339844 84.535156 53.34375 93.746094 47.191406 C 102.960938 41.035156 112.683594 35.835938 122.921875 31.59375 C 133.160156 27.355469 143.710938 24.152344 154.578125 21.992188 C 165.445312 19.832031 176.417969 18.75 187.5 18.75 C 198.582031 18.75 209.554688 19.832031 220.421875 21.992188 C 231.289062 24.152344 241.839844 27.355469 252.078125 31.59375 C 262.316406 35.835938 272.039062 41.035156 281.253906 47.191406 C 290.464844 53.34375 298.988281 60.339844 306.824219 68.175781 C 314.660156 76.011719 321.65625 84.535156 327.808594 93.746094 C 333.964844 102.960938 339.164062 112.683594 343.40625 122.921875 C 347.644531 133.160156 350.847656 143.710938 353.007812 154.578125 C 355.167969 165.445312 356.25 176.417969 356.25 187.5 Z M 356.25 187.5 " />
        </g>
        <path style="stroke:none;fill-rule:nonzero;fill:#fff;fill-opacity:1;" d="M 145.648438 303.871094 L 53.625 211.851562 L 90.289062 175.179688 L 145.648438 230.53125 L 284.6875 91.480469 L 321.367188 128.144531 Z M 145.648438 303.871094 " />
    </g>
</svg>`,
        description: "Middleware to validate user input that interacts with the API",
        link: "https://express-validator.github.io/docs/",
        end: "back-end"
    },
    {
        name: "Render",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" d="M18.263.007c-3.121-.147-5.744 2.109-6.192 5.082c-.018.138-.045.272-.067.405c-.696 3.703-3.936 6.507-7.827 6.507a7.9 7.9 0 0 1-3.825-.979a.202.202 0 0 0-.302.178V24H12v-8.999c0-1.656 1.338-3 2.987-3h2.988c3.382 0 6.103-2.817 5.97-6.244c-.12-3.084-2.61-5.603-5.682-5.75"/></svg>',
        description: "The hosting service that powers the API",
        link: "https://render.com/",
        end: "back-end"
    },
    {
        name: "Netlify",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="#00bfa5" d="m11.25 2.232l-.127.127v4.567l.127.129h1.526l.126-.13V2.36l-.126-.127H11.25zM6.69 5.455L5.643 6.5v.21l1.6 1.6h1.108l.148-.148V7.055l-1.6-1.6zM1.026 11.11l-.127.127v1.528l.127.127h6.09l.127-.127v-1.528l-.127-.127zm15.858 0l-.127.127v1.528l.127.127h6.09l.127-.127v-1.528l-.127-.127zm-9.64 4.58l-1.6 1.6v.21l1.045 1.046h.21l1.6-1.6v-1.109l-.148-.146zm4.005 1.256l-.127.13v4.566l.127.127h1.526l.126-.127v-4.567l-.126-.129z"/><path fill="#cfd8dc" d="M14.855 15.172h-1.523l-.127-.127V11.48c0-.634-.249-1.125-1.013-1.142c-.394-.01-.844 0-1.325.019l-.072.074v4.61l-.127.128H9.146l-.128-.127V8.956l.128-.127h3.425a2.41 2.41 0 0 1 2.41 2.41v3.806z"/></svg>',
        description: "The hosting platform that serves the front end",
        link: "https://netlify.com",
        end: "front-end"
    },
    {
        name: "Fully responsive",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" d="M4 6v10h5v-4a2 2 0 0 1 2-2h5a2 2 0 0 1 2 2v4h2V6zM0 20v-2h4a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2h4v2h-6a2 2 0 0 1-2 2h-5a2 2 0 0 1-2-2zm11.5 0a.5.5 0 0 0-.5.5a.5.5 0 0 0 .5.5a.5.5 0 0 0 .5-.5a.5.5 0 0 0-.5-.5m4 0a.5.5 0 0 0-.5.5a.5.5 0 0 0 .5.5a.5.5 0 0 0 .5-.5a.5.5 0 0 0-.5-.5M13 20v1h1v-1zm-2-8v7h5v-7z"/></svg>',
        description: "Available for all screen sizes!",
        link: "#",
        end: "front-end"
    },
    {
        name: "Postman",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128"><path fill="#f37036" d="M113.117 26.066C92.168-1.062 53.191-6.07 26.062 14.883c-27.125 20.953-32.128 59.93-11.175 87.055c20.957 27.124 59.937 32.124 87.058 11.167c27.114-20.953 32.118-59.918 11.172-87.039m0 0"/><path fill="#fff" d="M91.078 24.164a10.04 10.04 0 0 0-5.781 2.426a10.03 10.03 0 0 0-1.54 13.465a10.03 10.03 0 0 0 13.276 2.715h.002v.001l.156.155a10.6 10.6 0 0 0 1.965-1.45A10.34 10.34 0 0 0 99 27.107v-.002l-8.844 8.789l-.156-.155l8.844-8.793a10.04 10.04 0 0 0-7.766-2.78zM79.434 38.551c-4.24-.007-11.163 4.799-28.067 21.703l.084.086c-.092-.032-.185-.035-.185-.035l-6.364 6.308a1.035 1.035 0 0 0 .93 1.762l10.914-2.328a.3.3 0 0 0 .092-.17l.242.25l-3.72 3.69h-.18l-22.086 22.26l7.086 6.824a1.25 1.25 0 0 0 1.476.149a1.33 1.33 0 0 0 .645-1.356l-1.035-4.5a.53.53 0 0 1 0-.62a117.3 117.3 0 0 0 26.738-17.583l-4.535-4.537l.086-.014l-2.69-2.689l.172-.174l.182.186l-.094.091l7.137 7.293v-.003c13.68-12.954 23.39-23.367 20.865-30.375a3.83 3.83 0 0 0-1.107-2.208v.004a4 4 0 0 0-.483-.306c-.083-.088-.156-.178-.244-.264l-.066.066a4 4 0 0 0-.582-.29l.289-.292c-1.796-1.6-3.28-2.924-5.5-2.93zM30.94 92.21l-5.171 5.172v.004a1.03 1.03 0 0 0-.457 1.125a1.035 1.035 0 0 0 .921.789l12.672.875z"/><path fill="#f37036" d="M91.95 23.31a11.05 11.05 0 0 0-7.759 3.17a10.99 10.99 0 0 0-2.39 11.641c-4.741-2.03-11.155 1.51-31.106 21.457a1 1 0 0 0-.037.094a1 1 0 0 0-.119.062l-6.309 6.364a1.97 1.97 0 0 0-.363 2.324a2.01 2.01 0 0 0 1.707.984l.313-.203l8.424-1.797l-4.03 4.067a1 1 0 0 0-.054.166l-19.75 19.799a.8.8 0 0 0-.192.238l-5.086 5.09a1.97 1.97 0 0 0-.414 2.043a2 2 0 0 0 1.656 1.265l12.618.88a1 1 0 0 0 .52-.415a.89.89 0 0 0 0-1.035l-.026-.025a2.2 2.2 0 0 0 .705-.58a2.24 2.24 0 0 0 .406-1.876l-.984-4.187a126.7 126.7 0 0 0 26.334-16.861a1 1 0 0 0 .248.103c.254-.019.492-.128.672-.308c13.55-12.83 21.515-21.622 21.515-28.602a8 8 0 0 0-.431-2.85a11 11 0 0 0 3.845.83l-.015.004a11.2 11.2 0 0 0 5.183-1.45l.004.001a.84.84 0 0 0 .617-.055a9.4 9.4 0 0 0 2.07-1.652a10.87 10.87 0 0 0 3.258-7.758a10.87 10.87 0 0 0-3.257-7.758a1 1 0 0 0-.118-.091a11.05 11.05 0 0 0-7.656-3.078zm-.087 1.772a9.27 9.27 0 0 1 5.586 1.914l-8.068 8.117a1 1 0 0 0-.076.098a.83.83 0 0 0-.239.55a.83.83 0 0 0 .313.65h.002l6.1 6.1a9.04 9.04 0 0 1-10.028-1.913c-2.586-2.6-3.336-6.504-1.953-9.891c1.383-3.39 4.68-5.605 8.363-5.625m7.12 3.432a8.87 8.87 0 0 1 2.033 5.674a9.15 9.15 0 0 1-2.688 6.464a10 10 0 0 1-1.098.895L92.307 36.7l-.963-.963l.265-.265l7.373-6.96zm-.366 4.193a.78.78 0 0 0-.55.031a.73.73 0 0 0-.36.426a.73.73 0 0 0 .05.559a2.23 2.23 0 0 1-.257 2.328a.64.64 0 0 0-.195.488c.004.184.07.36.195.492a.58.58 0 0 0 .414 0a.68.68 0 0 0 .672-.207a3.57 3.57 0 0 0 .465-3.777v.004a.78.78 0 0 0-.434-.344M79.34 39.43a5.6 5.6 0 0 1 3.31 1.226a4.76 4.76 0 0 0-2.681 1.34L57.162 64.701l-4.476-4.476c11.828-11.772 19.06-17.921 23.556-19.936a5.6 5.6 0 0 1 3.098-.86zm3.965 2.96a2.9 2.9 0 0 1 2.043.844a2.8 2.8 0 0 1 .879 2.121a2.87 2.87 0 0 1-.985 2.07l-24.25 21.106l-2.617-2.617l22.887-22.68a2.9 2.9 0 0 1 2.043-.843zm2.994 6.698c-1.69 6.702-10.647 15.783-19.987 24.607l-3.777-3.773zM51.367 61.547l.274.27l3.513 3.513l-9.63 2.06zm5.793 5.84l.004.004l1.168 1.195l.018.084l.078.012l.248.254l.82.84l-5.385.66zm3.867 4.076l3.578 3.576A127 127 0 0 1 38.75 91.695a1.44 1.44 0 0 0-.777 1.653l1.035 4.5a.31.31 0 0 1 0 .363a.31.31 0 0 1-.414 0l-6.102-6.152L51.3 72.975zm-29.933 21.94l.869.814l4.492 4.492l-10.016-.648l4.655-4.659z"/></svg>',
        description: "An API development tool.",
        link: "https://www.postman.com/",
        end: "back-end"
    },
    {
        name: "Neon",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256"><defs><linearGradient id="logosNeonIcon0" x1="100%" x2="12.069%" y1="100%" y2="0%"><stop offset="0%" stop-color="#62f755"/><stop offset="100%" stop-color="#8ff986" stop-opacity="0"/></linearGradient><linearGradient id="logosNeonIcon1" x1="100%" x2="40.603%" y1="100%" y2="76.897%"><stop offset="0%" stop-opacity="0.9"/><stop offset="100%" stop-color="#1a1a1a" stop-opacity="0"/></linearGradient></defs><path fill="#00e0d9" d="M0 44.139C0 19.762 19.762 0 44.139 0H211.86C236.238 0 256 19.762 256 44.139v142.649c0 25.216-31.915 36.16-47.388 16.256l-48.392-62.251v75.484c0 21.939-17.784 39.723-39.722 39.723h-76.36C19.763 256 0 236.238 0 211.861zm44.139-8.825a8.817 8.817 0 0 0-8.825 8.818v167.73c0 4.878 3.946 8.831 8.818 8.831h77.688c2.44 0 3.087-1.977 3.087-4.416v-101.22c0-25.222 31.914-36.166 47.395-16.255l48.391 62.243V44.14c0-4.879.455-8.825-4.416-8.825z"/><path fill="url(#logosNeonIcon0)" d="M0 44.139C0 19.762 19.762 0 44.139 0H211.86C236.238 0 256 19.762 256 44.139v142.649c0 25.216-31.915 36.16-47.388 16.256l-48.392-62.251v75.484c0 21.939-17.784 39.723-39.722 39.723h-76.36C19.763 256 0 236.238 0 211.861zm44.139-8.825a8.817 8.817 0 0 0-8.825 8.818v167.73c0 4.878 3.946 8.831 8.818 8.831h77.688c2.44 0 3.087-1.977 3.087-4.416v-101.22c0-25.222 31.914-36.166 47.395-16.255l48.391 62.243V44.14c0-4.879.455-8.825-4.416-8.825z"/><path fill="url(#logosNeonIcon1)" fill-opacity="0.4" d="M0 44.139C0 19.762 19.762 0 44.139 0H211.86C236.238 0 256 19.762 256 44.139v142.649c0 25.216-31.915 36.16-47.388 16.256l-48.392-62.251v75.484c0 21.939-17.784 39.723-39.722 39.723h-76.36C19.763 256 0 236.238 0 211.861zm44.139-8.825a8.817 8.817 0 0 0-8.825 8.818v167.73c0 4.878 3.946 8.831 8.818 8.831h77.688c2.44 0 3.087-1.977 3.087-4.416v-101.22c0-25.222 31.914-36.166 47.395-16.255l48.391 62.243V44.14c0-4.879.455-8.825-4.416-8.825z"/><path fill="#63f655" d="M211.861 0C236.238 0 256 19.762 256 44.139v142.649c0 25.216-31.915 36.16-47.388 16.256l-48.392-62.251v75.484c0 21.939-17.784 39.723-39.722 39.723a4.41 4.41 0 0 0 4.409-4.409V115.058c0-25.223 31.914-36.167 47.395-16.256l48.391 62.243V8.825c0-4.871-3.953-8.825-8.832-8.825"/></svg>',
        description: "The database hosting provider for this project.",
        link: "https://www.neon.com",
        end: "back-end"
    },
    {
        name: "Typescrypt",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256"><path fill="#3178c6" d="M20 0h216c11.046 0 20 8.954 20 20v216c0 11.046-8.954 20-20 20H20c-11.046 0-20-8.954-20-20V20C0 8.954 8.954 0 20 0"></path><path fill="#fff" d="M150.518 200.475v27.62q6.738 3.453 15.938 5.179T185.849 235q9.934 0 18.874-1.899t15.678-6.257q6.738-4.359 10.669-11.394q3.93-7.033 3.93-17.391q0-7.51-2.246-13.163a30.8 30.8 0 0 0-6.479-10.055q-4.232-4.402-10.149-7.898t-13.347-6.602q-5.442-2.245-9.761-4.359t-7.342-4.316q-3.024-2.2-4.665-4.661t-1.641-5.567q0-2.848 1.468-5.135q1.469-2.288 4.147-3.927t6.565-2.547q3.887-.906 8.638-.906q3.456 0 7.299.518q3.844.517 7.732 1.597a54 54 0 0 1 7.558 2.719a41.7 41.7 0 0 1 6.781 3.797v-25.807q-6.306-2.417-13.778-3.582T198.633 107q-9.847 0-18.658 2.115q-8.811 2.114-15.506 6.602q-6.694 4.49-10.582 11.437Q150 134.102 150 143.769q0 12.342 7.127 21.06t21.638 14.759a292 292 0 0 1 10.625 4.575q4.924 2.244 8.509 4.66t5.658 5.265t2.073 6.474a9.9 9.9 0 0 1-1.296 4.963q-1.295 2.287-3.93 3.97t-6.565 2.632t-9.2.95q-8.983 0-17.794-3.151t-16.327-9.451m-46.036-68.733H140V109H41v22.742h35.345V233h28.137z"></path></svg>',
        description: "Stryctyle typed programming language that wraps Javascript to provide type checking",
        link: "https://www.typescriptlang.org/",
        end: "both-ends"
    },
    {
        name: "Jest Matchers",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 283"><path fill="#99425b" d="M239.553 138.96c0-14.508-11.762-26.27-26.27-26.27q-1.4.001-2.762.146l36.082-106.42H93.267l36.032 106.327a26 26 0 0 0-1.597-.052c-14.508 0-26.27 11.76-26.27 26.27c0 11.727 7.686 21.656 18.293 25.034c-5.602 9.704-12.652 18.445-20.656 25.997c-10.884 10.267-23.34 18.053-36.812 22.85c-16.04-8.24-23.614-26.565-16.78-42.632c.772-1.815 1.56-3.654 2.313-5.485c11.62-2.712 20.28-13.129 20.28-25.576c0-14.509-11.762-26.27-26.271-26.27s-26.27 11.761-26.27 26.27c0 8.897 4.426 16.756 11.193 21.508c-6.502 14.804-16.06 30.298-19.038 48.255c-3.569 21.52-.012 44.364 18.601 57.265c43.348 30.044 90.402-18.625 140.014-30.86c17.98-4.433 37.717-3.719 53.558-12.893c11.896-6.89 19.739-18.309 21.964-31.128c2.2-12.664-1.006-25.273-8.378-35.497a26.16 26.16 0 0 0 6.11-16.839"></path><path fill="#fff" d="M235.5 190.198c-1.932 11.13-8.806 20.851-18.858 26.674c-9.371 5.427-20.6 7.047-32.49 8.761c-6.369.918-12.955 1.869-19.389 3.455c-17.012 4.195-33.49 12.38-49.424 20.295c-31.89 15.84-59.43 29.518-85.398 11.52c-19.022-13.183-18.144-37.575-15.928-50.941c2.054-12.384 7.607-23.789 12.976-34.818a797 797 0 0 0 2.722-5.634a32.5 32.5 0 0 0 8.443 2.118c-5.466 18.039 3.13 37.653 21.171 46.921l2.47 1.269l2.614-.931c14.263-5.08 27.406-13.23 39.063-24.229c7.535-7.109 14.014-14.974 19.339-23.378a33 33 0 0 0 4.891.367c16.559 0 30.271-12.379 32.393-28.367c6.778-.248 13.977-.25 20.794-.002c2.12 15.99 15.834 28.37 32.393 28.37c6.924 0 13.346-2.17 18.637-5.856c3.673 7.519 5.044 15.979 3.581 24.406M41.8 119.295c10.948 0 19.854 8.907 19.854 19.855a19.73 19.73 0 0 1-3.135 10.687c-3.3 5.147-8.91 8.675-15.355 9.115c-.451.03-.906.052-1.364.052c-2.44 0-4.777-.445-6.938-1.253a19.97 19.97 0 0 1-10.305-8.777a19.7 19.7 0 0 1-2.612-9.824c0-10.948 8.907-19.855 19.855-19.855m66.049 19.666c0-9.947 7.353-18.206 16.907-19.634q1.444-.219 2.947-.22c6.365 0 12.032 3.015 15.666 7.687a20 20 0 0 1 2.318 3.765a19.7 19.7 0 0 1 1.87 8.402c0 1.587-.192 3.13-.545 4.61c-1.935 8.1-8.842 14.292-17.306 15.144q-.987.1-2.003.1a19.73 19.73 0 0 1-10.76-3.181c-5.466-3.541-9.094-9.69-9.094-16.673m82.865-23.61a32.7 32.7 0 0 0-8.991 15.117a305 305 0 0 0-22.458 0c-1.68-6.232-5.162-11.73-9.847-15.906l20.545-41.542zM102.216 12.834h135.437l-31.989 94.345a32.5 32.5 0 0 0-4.051 1.255l-31.6-64.463l-31.69 64.078a32.5 32.5 0 0 0-4.226-1.139zm130.92 126.128c0 1.99-.297 3.912-.844 5.727a19.93 19.93 0 0 1-7.653 10.545a19.74 19.74 0 0 1-11.357 3.582c-9.167 0-16.9-6.247-19.174-14.707a19.8 19.8 0 0 1-.68-5.147c0-2.714.548-5.302 1.538-7.66a20 20 0 0 1 1.878-3.442c3.573-5.274 9.602-8.752 16.438-8.752q.94.001 1.858.09c10.08.939 17.997 9.442 17.997 19.764m12.833 0c0-15.958-11.498-29.273-26.642-32.117L255.553 0H84.318L120.6 107.061c-14.618 3.253-25.583 16.316-25.583 31.9c0 11.702 6.184 21.98 15.452 27.756c-4.458 6.642-9.744 12.892-15.802 18.608c-9.608 9.063-20.311 15.937-31.85 20.46c-11.623-7.301-16.536-21.07-11.434-33.064l.4-.94l.75-1.764c12.766-4.453 21.953-16.603 21.953-30.867c0-18.024-14.663-32.687-32.686-32.687c-18.024 0-32.687 14.663-32.687 32.687c0 9.097 3.74 17.335 9.758 23.267a553 553 0 0 1-3.418 7.11c-5.503 11.301-11.739 24.11-14.098 38.335c-4.684 28.25 2.872 50.833 21.277 63.588c11.168 7.74 22.51 10.784 33.923 10.784c21.49 0 43.23-10.797 64.493-21.359c15.313-7.605 31.148-15.47 46.789-19.328c5.82-1.435 11.808-2.299 18.148-3.213c12.573-1.814 25.573-3.689 37.089-10.358c13.349-7.732 22.486-20.702 25.07-35.582c2.218-12.775-.451-25.57-6.961-36.422a32.5 32.5 0 0 0 4.787-17.011"></path></svg>',
        description: "A set of methods provided by Jest to make assertions about values being tested",
        link: "https://jestjs.io/docs/using-matchers",
        end: "back-end",
    },
    {
        name: "Supertest",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128"><circle cx={109.77} cy={11.02} r={4.66} fill="#00edcd"></circle><path fill="#81d4fa" d="m11.43 91.09l72.66-72.66c.11-.11.21-.28.31-.45c.77-1.32.85-2.58.73-4.25c-.21-2.83.46-4.36 1.83-5.38c1.81-1.36 8.08-1.08 19.81 10.65s12 17.99 10.64 19.8c-1.02 1.37-2.55 2.03-5.38 1.83c-1.67-.12-2.93-.05-4.25.73c-.18.1-.34.2-.45.31l-72.66 72.66c-9.98 9.98-19.51 8.46-25.56 2.41l-.04-.04l-.04-.04c-6.07-6.06-7.59-15.59 2.4-25.57" opacity={0.75}></path><path fill="#1d44b3" d="M99.79 23.22c-6.36-6.36-11.2-9.55-14.69-10.98c-.02.45-.02.95.02 1.49c.09 1.19.06 2.17-.23 3.11c3.17 1.71 7.14 4.55 12.01 9.11c7 6.56 10.34 11.51 11.73 15c.92-.34 1.86-.4 2.98-.35c-.67-3.5-3.62-9.19-11.82-17.38" opacity={0.39}></path><path fill="#00bfa5" d="M54.15 48.51L11.67 90.94c-10.36 10.35-9.05 20.03-3.03 26.05c6.02 6.01 15.68 7.35 26.06-3.03L102 47.04z"></path><circle cx={45.06} cy={76.96} r={5.06} fill="#00937a"></circle><circle cx={96.58} cy={37.04} r={4.76} fill="#00bfa5"></circle><circle cx={73.27} cy={57.3} r={2.93} fill="#fff" opacity={0.69}></circle><circle cx={121.84} cy={12.01} r={2.17} fill="#00bfa5"></circle><circle cx={59.15} cy={72.54} r={4} fill="#00edcd"></circle><circle cx={63.75} cy={61.12} r={1.54} fill="#00edcd"></circle><path fill="none" stroke="#fff" strokeLinecap="round" strokeMiterlimit={10} strokeWidth={4.195} d="M93.2 14.66c3.21 2.08 6.13 4.6 8.65 7.46" opacity={0.6}></path><path fill="#00edcd" d="M102.02 46.96c-.84.74-3.57 1.23-10.75 2.35c-3.81.6-8.47 1.2-13.82 1.64c-16.77 1.39-28.08-.9-21.37-3.64c5.39-2.2 24.53-1.72 36.51-1.34c5.78.19 10.09.42 9.43.99"></path><path fill="#fff" d="M11.63 103.5c-1.72-.71-1.02-2.68 0-3.71l67.5-69.29c4.1-4.24 7.85-7.12 9.95-5.12c2.35 2.23 1.6 6.23-7.61 14.49l-64.68 61.17c-.51.51-3.44 3.17-5.16 2.46" opacity={0.6}></path></svg>',
        description: "A library to test HTTP requests for Express",
        link: "https://www.npmjs.com/package/supertest",
        end: "back-end",
    },
    {
        name: "Vitest",
        icon: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><path fill="#689f38" d="M16.094 30.074a1.4 1.4 0 0 1-1.003-.416l-6.622-6.622a1.42 1.42 0 0 1 2.006-2.006l5.62 5.618l12.24-12.24a1.419 1.419 0 0 1 2.007 2.006L17.098 29.658a1.4 1.4 0 0 1-1.004.416"></path><path fill="#689f38" fillOpacity={0.502} d="M16.089 30.074a1.4 1.4 0 0 0 1.003-.416l6.622-6.622a1.42 1.42 0 0 0-2.006-2.006l-5.62 5.618l-12.24-12.24a1.42 1.42 0 0 0-2.007 2.006l13.244 13.244a1.4 1.4 0 0 0 1.004.416"></path><path fill="#ffca28" d="M24 10h-6V2l-8 12h6v8z"></path></svg>',
        description: "The testing framework used to run all the back-end tests",
        link: "https://vitest.dev/",
        end: "back-end"
    }
];

const About = () => {
    return <main className={styles["about"]}>
        <Background />
        <div
            className={styles["content"]}
        >
            <div className={styles["tools-section"]}>
                <h2 className={styles["title"]}>
                    Tools used to build this project
                </h2>
                <svg className={styles["icon"]} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
                    <g fill="currentColor">
                        <path d="M28.354 1.354a.5.5 0 0 0-.141-.805a5.98 5.98 0 0 0-6.798 1.191c-1.698 1.709-2.291 4.393-1.499 6.637L8.377 19.916a5.9 5.9 0 0 0-1.996-.336c-1.714 0-3.376.682-4.559 1.872a6.08 6.08 0 0 0-1.281 6.747a.498.498 0 0 0 .813.153l3.491-3.509c.23-.232.601-.263.793-.068l1.507 1.577c.219.221.209.562-.022.795l-3.478 3.5a.5.5 0 0 0 .141.804a6 6 0 0 0 2.591.588c1.572 0 3.065-.632 4.207-1.78c1.698-1.708 2.291-4.392 1.498-6.636l11.539-11.54a5.9 5.9 0 0 0 1.997.336c1.714 0 3.376-.682 4.559-1.872A6.08 6.08 0 0 0 31.458 3.8a.498.498 0 0 0-.813-.153l-3.491 3.509a.57.57 0 0 1-.801-.01l-1.499-1.499a.55.55 0 0 1-.001-.794zm-4.208 5l1.499 1.499c.599.602 1.625.605 2.218.009l2.93-2.944a5.09 5.09 0 0 1-1.324 4.926c-.997 1.003-2.399 1.577-3.85 1.577c-.683 0-1.33-.128-1.923-.38a.5.5 0 0 0-.549.106l-12 12a.5.5 0 0 0-.107.547c.803 1.91.314 4.375-1.163 5.861a4.96 4.96 0 0 1-4.985 1.258l2.941-2.96c.621-.625.631-1.594.029-2.198l-1.506-1.577c-.581-.585-1.614-.549-2.218.062l-2.93 2.944a5.09 5.09 0 0 1 1.324-4.926c.997-1.003 2.399-1.577 3.85-1.577c.683 0 1.33.128 1.923.38a.5.5 0 0 0 .549-.106l12-12a.5.5 0 0 0 .107-.547c-.803-1.91-.314-4.376 1.163-5.862a4.96 4.96 0 0 1 4.982-1.258l-2.96 2.961a1.546 1.546 0 0 0 0 2.205"></path>
                        <path d="M29.21 30.647a.595.595 0 0 1-.827-.002l-10.03-10a.501.501 0 0 0-.706.709l10.029 9.998c.3.301.698.467 1.122.467s.822-.166 1.128-.474l1.429-1.493a1.55 1.55 0 0 0-.001-2.206l-10-10a.5.5 0 0 0-.707.707l9.999 9.999c.229.23.229.564-.007.802zM5.854 3.146l-3-3a.5.5 0 0 0-.707 0l-2 2a.5.5 0 0 0 0 .707l3 3a.5.5 0 0 0 .707.001l.646-.647l7.646 7.646a.5.5 0 1 0 .708-.706L5.207 4.5l.646-.646a.5.5 0 0 0 .001-.708M3.5 4.793L1.207 2.5L2.5 1.207L4.793 3.5z"></path>
                    </g>
                </svg>
                <div className={styles["container"]}>
                    {tools.map((tool, index) => {
                        return <a
                            style={{ animationDelay: `${(index + 1) * 2}00ms` }}
                            className={styles["tool-card"]}
                            target="_blank"
                            href={tool.link}
                        >
                            <div
                                dangerouslySetInnerHTML={{ __html: tool.icon }}
                                className={styles["icon-container"]}
                            >
                            </div>
                            <div className={styles["left-pane"]}>
                            </div>
                            <div className={styles["right-pane"]}></div>
                            <div className={styles["card-content"]}>
                                <h3 className={styles["tool-name"]}>{tool.name}</h3>
                                <p className={styles["tool-description"]}>{tool.description}</p>
                                <span className={styles["end"]}>
                                    {tool.end.replace("-", " ")}
                                </span>
                            </div>
                        </a>
                    })}
                </div>
            </div>
            {sections.map((section, index) => {
                return <div
                    style={{ animationDelay: `${(index + 1) * 3}00ms` }}
                    className={styles["section"]}
                >
                    <div
                        className={styles["icon"]}
                        dangerouslySetInnerHTML={{ __html: section.icon }}
                    ></div>
                    <h2>{section.title}</h2>
                    <p className={styles["details"]}>
                        {section.details}
                    </p>
                </div>
            })}
            <div className={styles["section"]}>
                <div className={styles["icon"]}>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" fillRule="evenodd" d="M12 24c6.63 0 12-5.37 12-12S18.63 0 12 0S0 5.37 0 12s5.37 12 12 12M1 12C1 5.92 5.92 1 12 1c6.07 0 11 4.92 11 11c0 4.86-3.15 8.98-7.53 10.4c-.547.109-.747-.237-.747-.528l.005-.614c.005-.568.013-1.42.013-2.41c0-1.04-.346-1.69-.747-2.04c2.46-.273 5.03-1.2 5.03-5.43c0-1.2-.419-2.19-1.13-2.95c.109-.273.492-1.4-.109-2.91c0 0-.93-.291-3.03 1.13a10.6 10.6 0 0 0-2.75-.364c-.93.001-1.86.124-2.75.364c-2.1-1.42-3.03-1.13-3.03-1.13c-.602 1.51-.219 2.64-.11 2.91a4.3 4.3 0 0 0-1.13 2.95c0 4.23 2.57 5.17 5.01 5.43c-.31.273-.602.765-.693 1.48c-.638.292-2.22.765-3.21-.911c0 0-.583-1.06-1.7-1.13c0 0-1.08 0-.073.674c0 0 .729.328 1.22 1.6c0 0 .638 1.97 3.7 1.31c0 .596.008 1.18.013 1.58l.005.463c0 .291-.2.638-.747.528c-4.37-1.46-7.53-5.57-7.53-10.4z" clipRule="evenodd"></path></svg>
                </div>
                <h2>
                    Github
                </h2>
                <div className={styles["github-links"]}>
                    <a
                        className={`${styles["link"]} ${styles["profile"]}`}
                        href="https://github.com/IgnacioGandur/"
                        about="_blank"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><path fill="currentColor" d="M5.688 5S5 6.207 5 8.25c0 1.355.309 2.07.531 2.438C3.97 12.312 3 14.515 3 17.343C3 24.586 7.59 27 16 27s13-2.414 13-9.656c0-2.926-.914-5.125-2.406-6.719A7.83 7.83 0 0 0 26.344 5c-3.102 0-5.688 2.383-5.782 2.469A21.5 21.5 0 0 0 16 7c-1.605 0-3.18.195-4.656.563C9.266 5.502 6.758 5 5.688 5m15 10.438c.843-.008 1.625.062 2.312.28c1.832.583 3 2.266 3 4.157C26 25.109 23.191 26 16 26c-5.344 0-9.688-.941-9.688-5.938c0-1.906 1.036-2.984 1.782-3.656c1.64-1.476 4.23-.75 7.906-.75c1.672 0 3.277-.203 4.688-.219zM10.5 18c-.828 0-1.5.895-1.5 2s.672 2 1.5 2s1.5-.895 1.5-2s-.672-2-1.5-2m11 0c-.828 0-1.5.895-1.5 2s.672 2 1.5 2s1.5-.895 1.5-2s-.672-2-1.5-2m-11.156.844c.234 0 .406.199.406.437c0 .239-.172.438-.406.438a.444.444 0 0 1-.438-.438c0-.238.203-.437.438-.437m11 0c.234 0 .406.199.406.437c0 .239-.172.438-.406.438a.444.444 0 0 1-.438-.438c0-.238.203-.437.438-.437"></path></svg>
                        <h3>Github Profile</h3>
                    </a>
                    <a
                        href="https://github.com/IgnacioGandur/wheres_waldo_front_end"
                        className={`${styles["link"]} ${styles["front-end"]}`}
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><g fill="none" stroke="currentColor" strokeWidth={1.5}><path d="M21 2H3v20h18z"></path><path d="M16.655 14.157a1.655 1.655 0 1 1 0 3.31a1.655 1.655 0 0 1 0-3.31ZM3 19.944l5.383-4.256L17.343 22M12.269 5h-6.27M18 8.344H6m15 3.219H3"></path></g></svg>
                        <h3>Front end</h3>
                    </a>
                    <a
                        href="https://github.com/IgnacioGandur/wheres_waldo_back_end"
                        className={`${styles["link"]} ${styles["back-end"]}`}
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><g fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeMiterlimit={10} strokeWidth={1}><path d="M22.5 5.5H.5v6h22zm-4-4h-14l-4 4h22zm1 6v2m-2-2v2m-2-2v2"></path><path d="M4.75 7.25L3.5 8.5l1.25 1.25L6 8.5zM22.5 11.5H.5v6h22zm-3 2v2m-2-2v2m-2-2v2"></path><path d="M4.75 13.25L3.5 14.5l1.25 1.25L6 14.5zM22.5 17.5H.5v6h22zm-3 2v2m-2-2v2m-2-2v2"></path><path d="M4.75 19.25L3.5 20.5l1.25 1.25L6 20.5z"></path></g></svg>
                        <h3>Back end</h3>
                    </a>
                </div>
            </div>
        </div>
    </main>
}

export default About;
